...
async def start_track_download(query, context, track_info, quality):
    ...
    try:
        ...
        if file_path:
            ...
            # Update message to show completion with download another button
            keyboard = [[InlineKeyboardButton("🔄 Download Another", callback_data="main_menu")]]
            await query.edit_message_text(
                f"✅ *Download Complete!*\n\n"
                f"🎶 **{track_info['name']}**\n"
                f"👨‍🎤 *by {track_info['artist']}*\n\n"
...
async def start_playlist_download(query, context, playlist_info, quality):
    ...
    # Final summary
    # Add download another button for playlist completion
    keyboard = [[InlineKeyboardButton("🔄 Download Another", callback_data="main_menu")]]
    await query.edit_message_text(
...
async def start_album_download(query, context, album_info, quality):
    ...
    # Final summary
    # Add download another button for album completion
    keyboard = [[InlineKeyboardButton("🔄 Download Another", callback_data="main_menu")]]
    await query.edit_message_text(
